...
from app.services.legacy_smart_cleaner import SmartDataCleaner as LegacySmartCleaner
from app.services.smart_data_cleaner import SmartDataCleaner  # Enterprise version
...
legacy_smart_cleaner = LegacySmartCleaner()  # Original Llama 3.1 cleaner
smart_cleaner = SmartDataCleaner()  # Enterprise AI-powered cleaner
ai_chart_selector = AIChartSelector()  # AI chart recommendation
dashboard_generator = IntelligentDashboardGenerator()  # AI dashboard generation
...
async def clean_data_smart(
    ...
    try:
        ...
        # Apply smart cleaning using LLM-recommended strategies (legacy version)
        cleaning_result = legacy_smart_cleaner.clean_data(df, llm_analysis)
...
# ============ NEW ENTERPRISE AI ENDPOINTS ============

@app.post("/api/enterprise/clean-data-smart")
async def enterprise_clean_data_smart(file: UploadFile = File(...)):
    """
    Enterprise Smart Data Cleaning - executes ALL AI recommendations automatically
    ...
    - Advanced outlier handling (cap, winsorize, transform)
    - Feature engineering
    """
    try:
        # Read file
        ...
        # Step 2: Execute ALL recommendations using Enterprise SmartDataCleaner
        ...
        if output_format == 'csv':
            ...
            media_type = "text/csv"
            filename = file.filename.replace('.csv', '_enterprise_cleaned.csv')
        else:
            cleaned_df.to_excel(output, index=False)
            media_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            filename = file.filename.replace('.xlsx', '_enterprise_cleaned.xlsx').replace('.xls', '_enterprise_cleaned.xlsx')
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename={filename}",
                "X-Cleaning-Report": json.dumps(cleaning_report)
            }
        )
...
@app.post("/api/ai/tableau-recommendations")
